...
var api_url = "https://swapi.dev/api/vehicles/";
...
async function updateShipInfo () {
    ...
    let rawTransport = data.results;
    let infoTransport = rawTransport.map(({name, passengers, crew}) => ({name, passengers, crew}));
    globalNext = data.next;
    globalBack = data.previous;

    enablePagination();
    removeDivs();
    setDivs(dataSize, infoTransport);
}
